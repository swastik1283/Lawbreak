...
import { getAllFilesMatching, readJsonFile, } from "@nomicfoundation/hardhat-utils/fs";
...
export class ArtifactManagerImplementation {
    ...
    async getBuildInfoPath(buildInfoId) {
        return path.join(this.#artifactsPath, BUILD_INFO_DIR_NAME, buildInfoId + ".json");
    }
    async getBuildInfoOutputPath(buildInfoId) {
        return path.join(this.#artifactsPath, BUILD_INFO_DIR_NAME, buildInfoId + ".output.json");
    }
    async clearCache() {
